...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	$stmp2 = $db->prepare("UPDATE users SET username = ?, fullname = ?, email=?, password = ? WHERE id = ?");
	...
	if($_POST["pwd"] == $_POST["username"]){
		...
		if(isset($_SESSION["user"]))
			$stmp2->execute(array($username, $fullname, $email, $password, $_SESSION["user"]["id"]));
		else
...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<?= $pass_error?"<h4 style='color:red'>Password are not equal</h4>":"" ?>
